...
public class CustomWidget extends JPanel implements MouseListener {
    ...
    private void notifyObservers() {
        ShapeEvent event = new ShapeEvent(hexagonSelect, octagonSelect);
    ...
    private void calculateVertices(int width, int height) {
        ...
        for(int i = 0; i < hexagonVertex.length; i++) {
            ...
            hexagonVertex[i].setLocation((width/3)*2.3 + Math.cos(theta * i) * 100,
                                             height/2 + Math.sin(theta * i) * 100);
        }
        for(int j = 0; j < octagonVertex.length; j++){
            theta = 2 * Math.PI/octagonVertex.length; 
            octagonVertex[j].setLocation((width/3) + Math.cos(theta * j)*100,
    ...
    public void mouseClicked(MouseEvent event) {
        ...
        if(shapes[0].contains(event.getX(), event.getY())) {
            hexagonSelect = !hexagonSelect;
            selected = !selected; 
            notifyObservers();
        }
        if(shapes[1].contains(event.getX(), event.getY())) {
            octagonSelect = !octagonSelect;
            selected = !selected; 
            notifyObservers();
        }
